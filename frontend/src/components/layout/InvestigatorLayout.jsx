...
import {
    RiDashboardLine, RiGroupLine, RiBarChart2Line,
    RiMenuLine, RiCloseLine, RiLogoutBoxLine, RiMicroscopeLine,
} from 'react-icons/ri'

const navItems = [
    { to: '/investigator', icon: RiDashboardLine, label: 'Overview', exact: true },
    { to: '/investigator/patients', icon: RiGroupLine, label: 'Patient Registry' },
    { to: '/investigator/analytics', icon: RiBarChart2Line, label: 'Analytics & Export' },
]
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex items-center gap-3 px-4 pt-6 pb-8">
                <div className="w-9 h-9 rounded-xl bg-luscious flex items-center justify-center flex-shrink-0">
                    <RiMicroscopeLine className="text-white" size={18} />
                </div>
                <div>
                    <span className="font-display text-lg text-whiteout leading-none block">DiabetesDiet</span>
                    <span className="text-[10px] text-jazz uppercase tracking-widest">Investigator Portal</span>
                </div>
                {onClose && (
            ...
            <nav className="flex-1 px-3 space-y-0.5">
                <p className="px-3 mb-2 text-[10px] font-semibold text-jazz/50 uppercase tracking-widest">Research</p>
            ...
            <div className="p-3 mt-4 border-t border-white/5">
                <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl mb-1">
                    <div className="w-8 h-8 rounded-full bg-luscious/40 flex items-center justify-center flex-shrink-0">
                        <span className="text-xs font-semibold text-jazz uppercase">
                            {user?.full_name?.charAt(0) || 'I'}
                        </span>
                    </div>
                    <div className="min-w-0">
                        <p className="text-sm font-medium text-whiteout truncate">{user?.full_name || 'Investigator'}</p>
                        <p className="text-[10px] text-jazz">Investigator</p>
                    </div>
                </div>
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                <header className="h-14 flex items-center gap-3 px-4 md:px-6 bg-white border-b border-jazz/10 flex-shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-xs font-semibold bg-luscious/10 text-luscious px-3 py-1 rounded-full">
                            Investigator Portal
                        </span>
                    </div>
                    <div className="flex-1" />
                    <div className="flex items-center gap-2 text-sm text-jazz">
                        <span className="hidden sm:inline">FNDDS 2021–2023 · ADA 2024</span>
